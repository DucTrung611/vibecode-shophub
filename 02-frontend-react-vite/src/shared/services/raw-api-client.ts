...
// `apiClient` (shared/services/api-client.ts) unwraps straight to `data`, discarding
// `meta` — fine for single-resource endpoints but paginated list endpoints need
// `meta.total` for shared/components/Pagination.tsx. `rawApiClient` shares the same
// baseURL/auth-header/401-refresh behavior but resolves with the FULL envelope
// ({ success, data, meta }) instead. Kept as its own axios instance (rather than
// editing api-client.ts's interceptor) to avoid conflicting with other in-flight work
// on that shared file.
